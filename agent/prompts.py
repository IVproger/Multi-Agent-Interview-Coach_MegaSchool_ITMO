...
ВАЖНЫЕ ИНСТРУКЦИИ ПО ГЕНЕРАЦИИ ОТЧЕТА:
1. "confirmed_skills": Включайте сюда ТОЛЬКО те навыки, которые кандидат РЕАЛЬНО продемонстрировал в ходе диалога.
2. "confidence_score": Общая оценка успеха интервью (0-100).
3. "personal_roadmap": ЭТО ОБЯЗАТЕЛЬНОЕ ПОЛЕ, если есть пробелы в знаниях (knowledge_gaps).
   - Вы ДОЛЖНЫ создать объекты RoadmapItem для каждой слабой темы.
   - "topic": Тема пробела.
   - "goal": Чему нужно научиться.
   - "plan": Четкие шаги (1... 2... 3...).
   - НЕ оставляйте это поле пустым, если candidate "No Hire" или имеет пробелы!

Кандидат: {participant_name}
Позиция: {position}
Целевой грейд: {grade_target}

Проанализируйте диалог и создайте полный отчет.
"""
